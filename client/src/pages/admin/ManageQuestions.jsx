...
export default function ManageQuestions() {
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      setAiIsMock(Boolean(res.data.isMock));
      setAiFallbackReason(res.data.fallbackReason || '');
      setAiModalOpen(true);
    } catch (e) {
      const payload = e.response?.data;
      if (payload?.data?.length) {
        setAiQuestions(payload.data);
        setAiIsMock(Boolean(payload?.isMock));
        setAiFallbackReason(payload?.fallbackReason || '');
        setAiModalOpen(true);
      } else {
    ...
    <Layout>
      ...
      <Modal isOpen={aiModalOpen} onClose={() => setAiModalOpen(false)} title="AI Generated Questions Preview">
        <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
          ...
          {aiIsMock && (
            <div className="rounded-xl p-3 border border-yellow-400 bg-yellow-500/10 text-sm" style={{ color: '#f59e0b' }}>
              Live AI generation was unavailable, so fallback-generated questions are shown. Review them carefully.
              {aiFallbackReason && (
